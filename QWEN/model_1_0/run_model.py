...
# -----------------------------------------
# 1️⃣  Fonction : prédire si l'email est IP
# -----------------------------------------

def predict_ip(subject: str, body: str, attachments_text: str = "") -> float:
    """Retourne la probabilité que l'email soit IP."""
    text = f"{subject}\n{body}\n{attachments_text}"
...
def analyze_email(email_id: int, subject: str, body: str):
    ...
    print(f"➡️ Probabilité IP: {prob_ip:.2f}")

    if prob_ip < 0.60:
        print("❌ Email NON IP (probabilité trop faible)")
        return {
            "email_id": email_id,
            "is_ip": False,
            "probability": prob_ip,
...
# -----------------------------------------
# 4️⃣ Mode “ligne de commande”
# -----------------------------------------

if __name__ == "__main__":
    print("=== Demo analyse d'un email ===")
